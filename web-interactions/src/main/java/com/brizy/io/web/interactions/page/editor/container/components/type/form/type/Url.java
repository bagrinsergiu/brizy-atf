...
import com.brizy.io.web.interactions.page.editor.container.components.toolbar.variations.form.type.UrlField;
...
public class Url implements FormField<UrlFieldProperty> {

    Locator elementLocator;
    UrlFieldToolbar toolbar;
    FormProperties formProperties;

    public Url(ItemLocators itemLocators, FormProperties formProperties, Frame frame, Locator elementLocator) {
        this.formProperties = formProperties;
        this.elementLocator = elementLocator;
        var toolbarLocators = itemLocators.getToolbar();
        this.toolbar = new UrlFieldToolbar(toolbarLocators, elementLocator, frame);
    }

    @Override
    public void setToolbarProperties(UrlFieldProperty properties) {
        elementLocator.click();
        toolbar.setProperties(properties);
    }

    @Override
    public void configureFormField(UrlFieldProperty properties) {
    ...
    protected class UrlFieldToolbar extends FormToolbar<UrlFieldProperty> {

        Supplier<UrlField> urlField;

        public UrlFieldToolbar(ToolbarLocators toolbar, Locator elementLocator, Frame frame) {
            super(toolbar, frame);
            this.urlField = () -> new UrlField(toolbar.getField(), elementLocator, frame);
        }
        ...
        @Override
        public AbstractField<UrlFieldProperty> getField() {
            return urlField.get();
        }
         ...
         *
         * @param urlFieldProperty - properties for the url field
         */
        @Override
        public void setProperties(UrlFieldProperty urlFieldProperty) {
            super.setProperties(formProperties.toBuilder()
                    .field(formProperties.getField().toBuilder()
                            .fields(List.of(urlFieldProperty))
                            .build()
                    )
                    .build());
        }
